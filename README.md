...
6. **Acts autonomously** through AI agents that scan risks, draft reports, and assess submission readiness
...
A dark-themed, responsive dashboard built with React and Recharts with five pages:
...
- **AI Agents** — three autonomous agents for proactive risk management
...
### Step 6: Agentic AI (`frontend/src/Agent.jsx`)

Unlike the AI Chat which only answers questions on demand, the **AI Agents page** acts autonomously — it scans all data, identifies risks, makes decisions, and generates ready-to-use outputs without any manual analysis.

We built three agents:

#### 🔍 Risk Scanner Agent
- Scans all site metrics across all 23 studies automatically
- Identifies specific risk signals per site (low DQI, high query count, missing pages, protocol deviations)
- Generates a **prioritized action plan** with Critical / High / Medium priority levels
- Assigns specific actions to CRA or DM with deadlines (Immediate / This Week / This Month)

**Example output:**
> PRIORITY: Critical — Study 24, Site 421, CHN — 20 missing pages, 24 open queries → CRA must conduct immediate site visit

#### 📝 CRA Report Generator
- Select any study and site from dropdowns
- Agent fetches all subject-level data for that site automatically
- Drafts a complete **professional CRA site visit report** including:
  - Site overview with key metrics
  - Critical findings with specific subject IDs
  - DQI score interpretation
  - Numbered action items assigned to CRA / DM / Site
  - Follow-up timeline with deadlines
- Report can be copied and sent directly — no manual writing required

#### ✅ Submission Readiness Agent
- Evaluates all 23 studies against submission criteria simultaneously
- Returns **GO / NO-GO / CONDITIONAL** decision for each study
- Identifies specific blocking issues (e.g. "DQI is 76.9, needs 80; 11% clean subjects, needs 70%")
- Lists exact actions required to reach GO status
- Estimates effort required per study

**This is true agentic AI** — the system doesn't wait to be asked. It proactively processes all available data, makes decisions, and produces outputs that would otherwise require hours of manual review by a data management team.
...
| Which sites require immediate attention? | Automatic risk flag + Risk Scanner Agent |
| Is the data clean enough for submission? | Submission Readiness Agent — GO/NO-GO per study |
...
| Can we automate CRA reporting? | CRA Report Generator Agent |
| Can readiness checks be automated? | Submission Readiness Agent |
...
| Agentic AI | Groq API — LLaMA 3.3 70B Versatile (3 autonomous agents) |
...
├── frontend/
│   └── src/
│       ├── App.jsx                ← React dashboard (5 pages)
│       ├── Agent.jsx              ← 3 autonomous AI agents
